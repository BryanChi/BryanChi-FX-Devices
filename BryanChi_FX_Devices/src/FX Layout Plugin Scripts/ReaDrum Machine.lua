...
local function ButtonDrawlist(splitter, name, color, a)
  ...
  if DND_MOVE_FX and r.ImGui_IsMouseHoveringRect(ctx, xs, ys, xe, ye) then
    ...
    r.ImGui_DrawList_AddRect(f_draw_list, xs - x_offset, ys - x_offset, xe + x_offset, ye + x_offset,
      (RDM_DnD_Move or CustomColorsDefault.RDM_DnD_Move), 2,
      nil, 2)
  end
  if DND_ADD_FX and r.ImGui_IsMouseHoveringRect(ctx, xs, ys, xe, ye) then
    local x_offset = 2
    r.ImGui_DrawList_AddRect(f_draw_list, xs - x_offset, ys - x_offset, xe + x_offset, ye + x_offset,
      (RDM_DnDFX or CustomColorsDefault.RDM_DnDFX), 2,
      nil, 2)
  end
...
local function DrawPads(loopmin, loopmax)
  ...
  for a = loopmin, loopmax do
    ...
    ButtonDrawlist(SPLITTER, pad_name,
      Pad[a] and (RDM_PadOn or CustomColorsDefault.RDM_PadOn) or (RDM_PadOff or CustomColorsDefault.RDM_PadOff), a)
    DndAddFX_TARGET(a)
    ...
    if Pad[a] then
      ...
      DrawListButton(SPLITTER, "M", mute_color == true and (RDM_Mute or CustomColorsDefault.RDM_Mute) or 0xff2222ff, nil,
        nil)
    else
...
if not FxdCtx.FX[FxdCtx.FXGUID[FX_Idx]].Collapse then
  ...
  for ci = 0, 8 * (hy - 18), hy - 15.5 do
    for bi = 0, 15, 5 do
      for i = 0, 15, 5 do
        r.ImGui_DrawList_AddRectFilled(f_draw_list, wx + x + i + 1, wy + y + bi + ci - 5, wx + x + 4 + i,
          wy + y - 1 + bi + ci,
