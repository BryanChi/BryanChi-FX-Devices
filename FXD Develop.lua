...
function AddKnob(ctx, label, labeltoShow, p_value, v_min, v_max, Fx_P, FX_Idx, P_Num, Style, Radius, item_inner_spacing,
    ...
    if FP.ModAMT then -- Draw modlines  circular
        for Macro, v in ipairs(MacroNums) do
            if FP.ModAMT[Macro] then
                ...
                r.ImGui_DrawList_PathStroke(draw_list, EightColors.HighSat_MidBright[Macro], nil, radius_outer * 0.1)
                r.ImGui_DrawList_PathClear(draw_list)
    ...
    if Trk.Prm.Assign and F_Tp == Trk.Prm.Assign and AssigningMacro then
        ...
        r.ImGui_DrawList_PathStroke(draw_list, EightColors.bgWhenAsgnModAct[AssigningMacro], nil, radius_outer * 0.1)
        r.ImGui_DrawList_PathClear(draw_list)
...
function Add_WetDryKnob(ctx, label, labeltoShow, p_value, v_min, v_max, FX_Idx)
    ...
    if FX[FxGUID].DeltaP_V ~= 1 then
        ...
        r.ImGui_DrawList_AddText(draw_list, center[1] - radius_outer / 2 + 1, center[2] - radius_outer / 2, 0xffffffff,
            'S')
    end
...
function AddDrag(ctx, label, labeltoShow, p_value, v_min, v_max, Fx_P, FX_Idx, P_Num, Style, Sldr_Width,
    ...
    if AssigningMacro ~= nil then
        r.ImGui_DrawList_AddRectFilled(draw_list, PosL, PosT, PosR, PosB, EightColors.bgWhenAsgnMod[AssigningMacro])
    end
...
function demo.PushStyle()
    if app.style_editor then
        ...
        local file_path = ConcatPath(r.GetResourcePath(), 'Scripts', 'ReaTeam Scripts', 'FX', 'BryanChi_FX Devices',
            'ThemeColors.ini')
        local file = io.open(file_path, 'r')
...
function Show_KBShortcutEditor()
    ...
    if open then
        ...
        if r.ImGui_Button(ctx, '+') then
            ...
            r.ImGui_SetNextWindowPos(ctx, x + w / 2.5, y + h / 2)
        ...
        if r.ImGui_BeginPopupModal(ctx, '##Type Key Popup', nil, r.ImGui_WindowFlags_NoTitleBar()|r.ImGui_WindowFlags_NoResize()) then
            r.ImGui_Text(ctx, 'Type Key')
...
function loop()
    ...
    if visible then
        ...
        if not r.ImGui_IsAnyItemActive(ctx) then
            for i, v in pairs(KB_Shortcut) do
                if not v:find('+') then --if shortcut has no modifier
                    if r.ImGui_IsKeyPressed(ctx, AllAvailableKeys[v]) then
                    ...
                    --msg(_G['r.ImGui_ModFlags_'..v:gsub('Shift')..'()' ])
                    ...
                    if Mods == Mod and r.ImGui_IsKeyPressed(ctx, AllAvailableKeys[Ltr]) then
                        local Cmd_Num = r.NamedCommandLookup(Command_ID[i])
                        r.Main_OnCommand(Cmd_Num, 0)
                    end
                end
        ...
        if LT_Track == nil then
            ...
            if Mods == Alt then r.JS_WindowMessage_Release(hwnd, "WM_SETCURSOR") end
                                ...
                                if r.ImGui_BeginChild(ctx, FX_Name .. FX_Idx, FX.WidthCollapse[FxGUID] or FX.Width[FXGUID[FX_Idx]] or DefaultWidth, 220, nil, r.ImGui_WindowFlags_NoScrollWithMouse() |r.ImGui_WindowFlags_NoScrollbar()) and not Hide then ----START CHILD WINDOW------
                                    ...
                                    function SyncWetValues()
                                        ...
                                        if SyncWetValues == true then
                                            Wet.P_Num[FX_Idx] = r.TrackFX_GetParamFromIdent(LT_Track, FX_Idx, ':wet')
                                    ...
                                    if not FX[FXGUID[FX_Idx]].Collapse and FindStringInTable(BlackListFXs, FX_Name) ~= true and FindStringInTable(SpecialLayoutFXs, FX_Name) == false then
                                        ...
                                        if r.ImGui_IsMouseHoveringRect(ctx, Win_L, Win_T, Win_R, Win_B) then
                                            if ClickOnAnyItem == nil and LBtnRel and AdjustPrmWidth ~= true and ModifierHeld == 0 and not SaveEditingPopupModal then
                                            ...
                                            for Band = 1, 24, 1 do
                                                if ProQ3.Band_UseState[Band] == 1 then
                                                    ...
                                                    if ProQ3['AdjustingQ' .. FXGUID[FX_Idx]] then
                                                        local MousePosX_AdjustingQ_CheckXpos, Y = r
                                                            .GetMousePosition()
                                                        if ModifierHeld == Shift then
                                                            WheelQFineAdj = 20
                                                        else
                            ...
                            if r.ImGui_BeginChildFrame(ctx, '##FX Layer at' .. FX_Idx .. 'OnTrack ' .. TrkID, FXLayeringWin_X + Pad, 220, r.ImGui_WindowFlags_NoScrollbar()) then
                                ...
                                for LayerNum, LyrID in pairs(FX[FxGUID].LyrID) do
                                    ...
                                    if FX[FxGUID][Fx_P] then
                                        ...
                                        if ClickOnSolo then
                                            Lyr.Solo[LyrID .. FxGUID] = r.TrackFX_GetParamNormalized(LT_Track,
                                                FX_Idx, 4 + (5 * (LyrID - 1)))
                                            if Lyr.Solo[LyrID .. FxGUID] == 1 then
                                        ...
                                        if ClickOnMute then
                                            Lyr.Mute[LyrID .. FxGUID] = r.TrackFX_GetParamNormalized(LT_Track,
                                                FX_Idx, 5 * (LyrID - 1))
